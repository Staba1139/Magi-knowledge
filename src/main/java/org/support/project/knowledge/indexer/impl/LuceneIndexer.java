...
public class LuceneIndexer implements Indexer {
    ...
    private void addDoc(IndexWriter writer, IndexingValue indexingValue) throws Exception {
        ...
        // type (IntPoint for range queries + StoredField for retrieval)
        doc.add(new IntPoint(FIELD_LABEL_TYPE, indexingValue.getType()));
        doc.add(new StoredField(FIELD_LABEL_TYPE, indexingValue.getType()));
        // id
        Field idField = new StringField(FIELD_LABEL_ID, indexingValue.getId(), Field.Store.YES);
        doc.add(idField);
        // template
        if (indexingValue.getTemplate() != null) {
            doc.add(new IntPoint(FIELD_LABEL_TEMPLATE, indexingValue.getTemplate()));
            doc.add(new StoredField(FIELD_LABEL_TEMPLATE, indexingValue.getTemplate()));
        }
        // タイトル
        ...
        // 時刻 (LongPoint for range queries + StoredField for retrieval + DocValues for sorting)
        doc.add(new LongPoint(FIELD_LABEL_TIME, indexingValue.getTime()));
        doc.add(new StoredField(FIELD_LABEL_TIME, indexingValue.getTime()));
        doc.add(new NumericDocValuesField(FIELD_LABEL_TIME, indexingValue.getTime()));
